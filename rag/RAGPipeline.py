...
class CompleteRAGPipeline:
    ...
    def process_query(
        ...
        for term, department in restricted_terms.items():
            if term in query_lower:
                if department not in accessible_departments:

                    self.audit_logger.log_warning(
                        f"RBAC BLOCKED: User attempted access to {department}"
                    )

                    return {
                        "answer": f"Access denied. You do not have permission to access {department.upper()} information.",
                        "confidence": "N/A",
